...
class Phyla(nn.Module):
    ...
    def load(self, checkpoint_file = None):
        ...
        self.to(self.device)
        for module in self.modul:
            if hasattr(module, 'tree_head'):
                module.tree_head = module.tree_head.to(self.device)
        return self
...
class Mamba_LM_Tree_HeadModel(nn.Module, GenerationMixin):
    ...
    def __init__(
        ...
        if config.calculation_method == 'attention':
            ...
            self.tree_head = nn.MultiheadAttention(config.d_model, 1, batch_first=True, device=device)
        ...
        if not logits:
            if self.config.calculation_method == 'attention':
                ...
                if self.deepspeed:
                    ...
                    sequence_rep, weights = self.tree_head(sequence_rep, memory_rep, memory_rep, attn_mask = x.to(sequence_rep.device)) # TODO: Comment out for big model evaluation
